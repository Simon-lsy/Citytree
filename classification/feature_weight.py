...
# Weight = tf.get_variable("Weight", shape=([2 * n_hidden, n_class]), initializer=tf.orthogonal_initializer())
...
for attr in range(5, 18):

    attr_index_list = list()
    index = attr
    attr_name = list(new_df)[index][4:]
    print(attr_name)
    while index < len(list(new_df)):
        attr_index_list.append(index)
        index += 13
    print(attr_index_list)
    data = new_df.iloc[:, attr_index_list].values
    ...
    with tf.Session() as sess:
        ...
        while step < training_iters:
            ...
            #     print(
            #         'Iter' + str(step * batch_size) + ',Loss= %.6f' % loss + ', Train Accurancy= %.5f' % acc)
        ...
        score_list.append(
            (attr_name, sess.run(accuracy, feed_dict={x: data_test.reshape([-1, n_step, n_input]), y: target_test}))
            )
